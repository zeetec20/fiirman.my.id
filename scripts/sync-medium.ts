...
const FEED_URL = "https://medium.com/feed/@jusles363";
...
function detectLanguage(code: string): string {
	...
	if (/^\s*\{[\s\S]*\}\s*$/.test(code) && /"[^"\\]+"\s*:/.test(head)) return "json";
	// YAML
	...
	// Python
	if (/\bdef\s+\w+\s*\(|^\s*import\s+\w+|^\s*from\s+\w+\s+import\b|print\(/m.test(head))
		return "python";
	// Go
	if (/^\s*package\s+\w+|^\s*func\s+\w+\s*\(/m.test(head)) return "go";
	// Dart
	if (/\bvoid\s+main\s*\(\s*\)\s*\{|Widget\s+build\s*\(/.test(head)) return "dart";
	// CSS — selector { prop: value; }
	if (/[.#]?\w+(?:\s*[.#:][\w-]+)*\s*\{[\s\S]*?[\w-]+\s*:\s*[^;}]+;[\s\S]*?\}/m.test(head))
		return "css";
	// TypeScript / JavaScript — broad net last
	if (/\b(const|let|var|function|=>|async|await|interface|type|export|import|class|extends)\b/.test(head))
		return "ts";
	return "text";
}
